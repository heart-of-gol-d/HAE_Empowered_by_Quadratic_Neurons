...
def gen_latent_size(input_size, random_state):
    '''
    Generate the size of the latent space according to the input size
    '''
    return random_state.randint(3, input_size // 2)

def gen_alpha_coef(random_state):
    '''
    Generate alpha coefficient to determine the difference between layers in term of size
    '''
    return round(random_state.uniform(0.2, 0.8), 2)
...
def build_data_loaders(nb_loaders, datas, batchs_size):
    ...
    for _ in range(nb_loaders):
        ...
        nb_samples_subset = random.randint(inf, sup) # 返回参数1和2之间的任意整数
        ...
        data_loaders.append(DataLoader(training_dataset, shuffle=True, batch_size=batch_size, drop_last=True))
